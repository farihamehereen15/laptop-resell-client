...
const AllUsers = () => {
    ...
    const { data: users = [], isLoading, refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://laptop-resell-server-cyan.vercel.app/users')
    ...
    const handleDeleteUser = (user) => {
        // console.log(user)
        fetch(`https://laptop-resell-server-cyan.vercel.app/users/${user._id}`, {
            method: "DELETE",
            headers: {
    ...
    const handleMakeAdmin = (id) => {
        fetch(`https://laptop-resell-server-cyan.vercel.app/users/admin/${id}`, {
            method: "PUT",
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
